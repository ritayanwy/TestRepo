...
def list_handler(response):
    ...
    TEST['request'][3]['data']['payload'].append(json.dumps({'account_id':account_id,'reference_folder_info':{'is_trash':0,'mailbox_path':mailbox_path},'conversation_id': email['conversation_id'],'resource_id':email['resource_id'],'destination_folder_info':{'is_trash':0,'mailbox_path':"[Gmail]/Trash"}}))
    global ResourceId
...
TEST = {
        'name': 'ActionSetAPI-Delete an email in Inbox',
        'request': [
            {
                'method': 'POST',
