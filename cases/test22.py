    ...
    if len(res['data']['list']) > 0 and len(res['data']['list'][0]['list']) > 0:
        ...
        for folder in res['data']['list'][0]['list']:
            ...
            if folder['name'] == 'Parent folder':
                for childfolder in res['data']['list'][0]['list'][index]['list']:
                    if len(folder[index]['list']) > 0:
                        TEST['request'][1]['data']['mailbox_path'] = childfolder['mailbox_path']
                        TEST['request'][2]['data']['mailbox_path'] = childfolder['mailbox_path']
                        TEST['request'][1]['data']['folder'] = childfolder['label']
                        print TEST['request'][1]['data']['folder']
                        break
...
TEST = {
        'name': 'sync child folder previews check',
        'request': [
            {
                'method': 'POST',
            ...
            },
            {
                ...
                'data': {'ts': int(time.time()),
                         'count': 0,
                         'query': 'child folder previews check',
                         'sync_hash': '{}'},
                'timeout': 60,
            },
            {
