...
def list_handler(response):
    ...
    for email in res['data']['list']:
        ...
        TEST['request'][3]['data']['payload'].append({'account_id':account_id,'reference_folder_info':{'is_trash':0,'mailbox_path':mailbox_path},'conversation_id': email['conversation_id'],'resource_id':email['resource_id'],'destination_folder_info':{'is_trash':0,'mailbox_path':"[Gmail]/Trash"}})
        global ResourceId
...
TEST = {
        'name': 'ActionSetAPI-Delete 15 emails in Inbox',
        'request': [
            {
                'method': 'POST',
