...
def list_handler(response):
    ...
    TEST['request'][3]['data']['payload'].append(json.dumps({'account_id':account_id,'reference_folder_info':{'is_trash':0,'mailbox_path':mailbox_path},'conversation_id': email['conversation_id'],'resource_id':email['resource_id'],'mime_message_id':email['mime_message_id'],'unique_message_id':str(account_id)+'-'+str(email['received_time'])+'-test' }))
    return True
    ...
    if len(res['data']['change']) > 0 :
        for changes in res['data']['change']:
            if changes['type'] == 'message_update':
                if -1 in changes['payload'][0]['folder_list']:
                    return True
...
TEST = {
        'name': 'ActionSetAPI-Star an email in Custom Child Folder(level 2) with change get',
        'request': [
            {
                'method': 'POST',
