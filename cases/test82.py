...
def list_handler(response):
    ...
    TEST['request'][2]['data']['payload'].append(json.dumps({'account_id':account_id,'resource_id':email['resource_id'],'is_body_html':1, 'is_tentative':0 }))
    TEST['request'][3]['data']['account_id'].append(account_id)
    TEST['request'][3]['data']['payload'].append(json.dumps({'account_id':account_id,'resource_id':email['resource_id'],'is_body_html':1}))
    return True

def acceptmeetingrequest_handler(response):
    print response.text
    res = json.loads(response.text)
    if res['error_code'] != 0:
        return False
...
def declinemeetingrequest_handler(response):
    print response.text
    res = json.loads(response.text)
    if res['error_code'] != 0:
        return False
...
TEST = {
        'name': 'ActionSetAPI- Accept meeting request.Then decline. Inbox email',
        'request': [
            {
                'method': 'POST',
            ...
            'data': {'action_token': int(time.time()),
                     'action_type': 'accept_meeting_request',
                     'payload': [],
                     'account_id': []
                    },
            'timeout': 60
        ...
        'response': [
            ...
            {
                'status_code': 200,
                'hooks': acceptmeetingrequest_handler
            },
            {
                'status_code': 200,
                'hooks': declinemeetingrequest_handler
            }
        ]
}
