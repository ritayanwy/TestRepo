...
def list_handler(response):
    ...
    TEST['request'][2]['data']['payload'].append(json.dumps({'account_id':account_id,'reference_folder_info':{'is_trash':0,'mailbox_path':mailbox_path},'conversation_id': email['conversation_id'],'resource_id':email['resource_id'] }))
    return True
...
TEST = {
        'name': 'ActionSetAPI-UnStar an email in Custom Folder(level 0)',
        'request': [
            {
                'method': 'POST',
