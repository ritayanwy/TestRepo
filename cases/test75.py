...
def list_handler(response):
    ...
    TEST['request'][3]['data']['payload'].append(json.dumps({'account_id':account_id,'reference_folder_info':{'is_trash':0,'mailbox_path':mailbox_path},'conversation_id': email['conversation_id'],'resource_id':email['resource_id'],'mime_message_id':email['mime_message_id'],'unique_message_id':str(account_id)+'-'+str(email['received_time'])+'-test','timezone_offset': 19800 ,'ts_reminder': int(time.time())+25 }))
    TEST['request'][4]['data']['account_id'].append(account_id)
    TEST['request'][4]['data']['payload'].append(json.dumps({'account_id':account_id,'reference_folder_info':{'is_trash':0,'mailbox_path':mailbox_path},'conversation_id': email['conversation_id'],'resource_id':email['resource_id'] }))
    return True
...
def changeget_handler1(response):
    ...
    hash1 = res['data']['hash']
    clv = res['data']['clv']
    TEST['request'][5]['data']['hash'] = hash1
    TEST['request'][5]['data']['clv'] = clv
    return True

def reminder_handler(response):
    print response.text
    res = json.loads(response.text)
    if res['error_code'] != 0:
        return False
...
def changeget_handler2(response):
    ...
    if len(res['data']['change']) > 0 :
        for changes in res['data']['change']:
            if changes['type'] == 'reminder_delete':
                return True
...
TEST = {
        'name': 'ActionSetAPI- Set reminder for Custom Child Folder(level 2) email, Unstar, Check reminder_delete with change get',
        'request': [
            {
                'method': 'POST',
        ...
        'response': [
            ...
            {
                'status_code': 200,
                'hooks': reminder_handler
            },
            {
                'status_code': 200,
                'hooks': unstar_handler
            },
            {
                'status_code': 200,
